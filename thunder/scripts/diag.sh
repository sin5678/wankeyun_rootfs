...
volume_get()
{
	...
	do
		...
		# check really write ?
		#echo "test write ${VOL}"
		DIAG_LOG2=${VOL}/onecloud-diag.tar.gz
...
diag_inc_find()
{
	...
	do
		...
		if [ -f ${NEED_DIAG} -o -d ${NEED_DIAG} ]; then
			needdiag=1
...
diag_network()
{
	...
	if [ "${IP_ADDR}" != "" -a "${GATEWAY}" != "" ]; then
		...
		echo "---------------------------------------->" >> ${DIAG_LOG}
		ping ntp.cc.sandai.net  -c 4 -W 1 -w 2 >> ${DIAG_LOG}
		echo "---------------------------------------->" >> ${DIAG_LOG}
		ping ntp.ubuntu.com -c 4 -W 1 -w 2 >> ${DIAG_LOG}
		echo "---------------------------------------->" >> ${DIAG_LOG}
		ping ubuntu.pool.ntp.org -c 4 -W 1 -w 2 >> ${DIAG_LOG}
		echo "---------------------------------------->" >> ${DIAG_LOG}
		ping cn.pool.ntp.org -c 4 -W 1 -w 2 >> ${DIAG_LOG}
		echo "---------------------------------------->" >> ${DIAG_LOG}
		ping time.asia.apple.com -c 4 -W 1 -w 2 >> ${DIAG_LOG}
	...
	if [ -f "/thunder/bin/netcat" ]; then
		echo "-----------------------ntp server----------------->" >> ${DIAG_LOG}
		echo "/thunder/bin/netcat -uzv ntp.cc.sandai.net 123" >>  ${DIAG_LOG}
		/thunder/bin/netcat -uvz ntp.cc.sandai.net 123 >>  ${DIAG_LOG} 2>&1
		echo "/thunder/bin/netcat -uzv ntp.cc.sandai.net 123" >>  ${DIAG_LOG}
		/thunder/bin/netcat -uvz ntp.ubuntu.com 123 >>  ${DIAG_LOG} 2>&1
		echo "/thunder/bin/netcat -uzv ntp.ubuntu.com 123" >>  ${DIAG_LOG}
		/thunder/bin/netcat -uvz ubuntu.pool.ntp.org 123 >>  ${DIAG_LOG} 2>&1
		echo "/thunder/bin/netcat -uzv ubuntu.pool.ntp.org 123" >>  ${DIAG_LOG}
		/thunder/bin/netcat -uvz ntp.cc.sandai.net 123 >>  ${DIAG_LOG} 2>&1
		echo "/thunder/bin/netcat -uzv cn.pool.ntp.org 123" >>  ${DIAG_LOG}
		/thunder/bin/netcat -uvz cn.pool.ntp.org 123 >>  ${DIAG_LOG} 2>&1
		echo "/thunder/bin/netcat -uzv time.asia.apple.com 123" >>  ${DIAG_LOG}
		/thunder/bin/netcat -uvz time.asia.apple.com 123 >>  ${DIAG_LOG} 2>&1
		...
		echo "---------------turnclient1: chinanet,chinaUnicom, CMCC------------------------->" >> ${DIAG_LOG}
		/thunder/bin/netcat -zv 180.101.141.69 5040 >>  ${DIAG_LOG} 2>&1
		/thunder/bin/netcat -zv 153.35.135.69 5040 >>  ${DIAG_LOG} 2>&1
		/thunder/bin/netcat -zv 223.111.217.69  5040 >>  ${DIAG_LOG} 2>&1
		echo "---------------turnclient2: chinanet,chinaUnicom, CMCC------------------------->" >> ${DIAG_LOG}
		/thunder/bin/netcat -zv 180.101.141.73 5040 >>  ${DIAG_LOG} 2>&1
		/thunder/bin/netcat -zv 153.35.135.73 5040 >>  ${DIAG_LOG} 2>&1
		/thunder/bin/netcat -zv 223.111.217.73 5040 >>  ${DIAG_LOG} 2>&1
...
diag_system()
{
	...
	echo "================== fdrawer.log.0==================" >> ${DIAG_LOG}
	echo "cat /tmp/fdrawer.log.0  :" >> ${DIAG_LOG}                                                                           
	cat /tmp/fdrawer.log.0   >> ${DIAG_LOG} 2>&1
	echo "================== ubus_app.log=================" >> ${DIAG_LOG}
	echo "All ubus log  message :" >> ${DIAG_LOG}
	cat /tmp/ubus_app.log  >> ${DIAG_LOG} 2>&1
	echo "================== ubus_app.log.0=================" >> ${DIAG_LOG}
	echo "All ubus log  message :" >> ${DIAG_LOG}
	cat /tmp/ubus_app.log.0  >> ${DIAG_LOG} 2>&1
	echo "================== turnclient.log=================" >> ${DIAG_LOG}
	echo "All turnclient log  message :" >> ${DIAG_LOG}
	cat /tmp/turnclient.log  >> ${DIAG_LOG} 2>&1
	echo "================== turnclient.log.0=================" >> ${DIAG_LOG}
	echo "All turnclient log  message :" >> ${DIAG_LOG}
	cat /tmp/turnclient.log.0  >> ${DIAG_LOG} 2>&1
	echo "================== dmesg  message=================" >> ${DIAG_LOG}	
	echo "dmesg:" >> ${DIAG_LOG}
	dmesg >> ${DIAG_LOG} 2>&1
...
diag_system
diag_disk
echo "======================system time data: ======================" >> ${DIAG_LOG}
date >> ${DIAG_LOG}
...
if [ ${diag_ok} -eq 1 ]; then
	...
	tar czvf ${DIAG_LOG2}  ${DIAG_LOG}_enc
	sync
	sleep 1
	rm -f ${DIAG_LOG}
	rm -rf  ${DIAG_LOG}_enc 
